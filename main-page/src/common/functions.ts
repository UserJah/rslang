...
const resp=await Promise.all(q.map(async (elem)=>{
  const resp1=await elem
  const body=await resp1.json()
  return body}))
return resp
}
export async function prepare(page=29,group = 0,fromPage?:boolean):Promise<WordSignature[]> {
  let arr1:Word[] = [];
 ...
 if (isUser && fromPage){
  ...
  const filtered=a.flat().filter(elem=>!known.some(element=>element.id===elem.id))
...
export async function fetchWords(page=0,group=0):Promise<Word[]> {
  const res = await fetch(`https://qwerzxvxzvzxvxzv.herokuapp.com/words?page=${page}&group=${group}`);
  const resp = await res.json();
  return resp;
}

function createTranslation(arr:Word[],element:Word,isTrue:boolean):string{
...
export function Score(correct:boolean,elem:WordSignature,data:{streak:number,score:number,arr:WordSignature[]}):void{
...
const rawResp=await fetch(baseurl,{method: 'GET',
headers: {
  'Authorization': `Bearer ${token}`,
  'Accept': 'application/json',
}
