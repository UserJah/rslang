...
import { pteroIMG,pidgeionIMG,turkeyIMG,penguinIMG } from '../common/assets/birdImages';

import './gameScreen.css'

export function Card(props:{items:WordSignature[],end:boolean,func:()=>void,reset:number}) {
...
const stars=function(strk:number,mult:number){
  ...
  for (let index = 0; index < 3; index++) {
    if (index<strk%mult) arr.push(<StarIconFilled sx={{color: "#FC0" ,fontSize: "60px"}} />)
...
return [arr[0]].concat(arr.slice(1,1 + (Math.floor(strk/mult)>5?4:Math.floor(strk/mult))))
}
...
if (!props.end) {
    ...
    <div
      dangerouslySetInnerHTML={{__html: stick}}
    />

        <Morebuttons elems={props.items}
