...
export function AudioChallenge(props?: {
  page?: number
  group?: number
  fromPage?: boolean
}) {
  ...
  useEffect(() => {
    const resp = async () => {
      const response = await prepareAudioChallenge(
        props?.page,
        props?.group,
        props?.fromPage
      )
  ...
  function reset() {
    setStatsData(() => {
      return { new: 0, answers: 0, correctAnswers: 0, learned: 0, bigStreak: 0 }
    })
    isFinish(false)
    setCounter(0)
    setGuessed([])
    ...
    setStatsData(
      (statsData) =>
        (statsData = {
          ...
          correctAnswers:
            statsData.correctAnswers + (items[counter].guessedCorrect ? 1 : 0),
          learned:
            statsData.learned +
            learned(a, items[counter].properties?.optional?.isKnown),
  ...
  function filterItems() {
    items.forEach(
      (element) => (element.variant = shuffle(element.variant as string[]))
    )
  ...
  if (!load && !finish) {
    ...
    return (
      <div className="gameboard">
        ...
        Вы отгадали все слова. Выйдите из игры для выбора другого уровня
        сложности или страницы учебника
      </div>
    )
  } else if (!finish) {
    return (
