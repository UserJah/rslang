...
export function Controls(props: {
  items: WordSignature[]
  updatefunc: (element: string) => void
  counter: number
  load: boolean
  updatecount: () => void
}) {
      ...
      if (!screen) {
        for (let i = 1; i < 6; i += 1) {
          if (evt.code === `Digit${i}`) {
            ...
            isScreen(true)
            props.updatefunc(
              (props.items[props.counter].variant as string[])[i - 1]
            )
          }
        }
      } else {
  ...
  if (!screen)
    return (
      <div>
        ...
        <div className="variants">
          {(props.items[props.counter].variant as string[]).map(
            (element, index) => {
              return (
                <button
                  key={props.items[props.counter].id + `${element}`}
                  onClick={() => {
                    setAnswer(element)
                    isScreen(true)
    ...
    return (
      <div>
        <div className="image">
          <img
            src={`https://qwerzxvxzvzxvxzv.herokuapp.com/${
              props.items[props.counter].image as string
            }`}
            alt=""
          />
        </div>
